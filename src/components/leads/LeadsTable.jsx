...
function SortIcon({ col, sortKey, sortDir }) {
  ...
  if (sortKey !== col.key) return <ArrowUpDown size={12} className="text-gray-300" />
  return sortDir === 'asc'
...
function Th({ col, sortKey, sortDir, onSort }) {
  return (
    <th
      className={`
        px-4 py-3 text-left text-[11px] font-semibold text-ink-muted uppercase tracking-wider
...
function LeadRow({ lead }) {
  return (
    <tr className="border-t border-gray-50 hover:bg-surface-secondary/60 transition-colors group">
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-medium text-ink leading-none">{lead.name}</p>
            <p className="text-[11px] text-ink-muted mt-0.5 font-mono">{lead.phone}</p>
          </div>
        </div>
      </td>

      {/* Phone — hidden on mobile since shown in Name cell */}
      <td className="px-4 py-3 hidden xl:table-cell">
        <span className="font-mono text-xs text-ink-secondary">{lead.phone}</span>
      </td>

      {/* Project */}
      <td className="px-4 py-3">
        <span className="text-sm text-ink-secondary" title={lead.project}>
      ...
      <td className="px-4 py-3 hidden sm:table-cell">
        <span className="font-mono text-sm font-semibold text-ink">{lead.value || '—'}</span>
      </td>
